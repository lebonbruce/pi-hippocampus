...
**强烈建议部署一个至少 8B 参数的本地模型。**

为什么？因为 V6.0 的两大核心特性 —— **肌肉记忆 (Muscle Memory)** 和 **全息联想搜索 (Holographic Search)** —— 必须依赖本地 LLM 的推理能力才能运行。

如果没有本地 LLM：
- ❌ **无法生成肌肉记忆**：你教给它的规则无法转化为"本能"。
- ❌ **无法进行联想检索**：当你说"无聊"时，它想不到要去查"计划"。
- ⚠️ **只能使用正则模式**：效果大打折扣，只能机械地记录关键词。
